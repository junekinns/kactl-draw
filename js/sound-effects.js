...
export function playResultPing(index) {
  safePlay((ctx) => {
    ...
    playTone(ctx, startTime, base, 0.18, 0.065, 'triangle');
    playTone(ctx, startTime + 0.01, base * 2, 0.12, 0.025, 'sine');
  });
}
...
export function playFinalCymbal() {
  safePlay((ctx) => {
    ...
    playNoiseBurst(ctx, startTime, 1.35, 0.28, 5200, 'highpass', 0.7);
    playNoiseBurst(ctx, startTime + 0.03, 0.85, 0.12, 7600, 'bandpass', 2.6);

    metallicFrequencies.forEach((frequency, i) => {
      playTone(ctx, startTime + i * 0.008, frequency, 0.42, 0.025, 'square');
    });

    playTone(ctx, startTime, 190, 0.18, 0.09, 'sine');
  });
}
